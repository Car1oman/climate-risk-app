...
export class Stage05Phenomena extends StageInterface {
  constructor() {
    ...
    this.rulesApplied = [
      ...
      "H-5.11 (documentacion-v2/stage-05, MEDIO): scenario se infiere via inferScenario() en lugar de hardcodear null. Hoy retorna null para todas las señales porque: (1) señales anomaly son observaciones actuales — no aplica escenario, (2) señales projected provienen de Open-Meteo CMIP6 HighResMIP ensemble (HALLAZGO-8, authoritative-sources.json) que es un pathway alto sin parámetro de selección SSP, (3) señales categóricas (ENSO) son estados discretos — no aplica escenario. Stage 6 usa fallback 'not_scenario_specific' (stage-06-risk.js:36-37), correcto y consistente con HALLAZGO-8. Para poblar scenario en el futuro: (a) agregar campo scenario a ClimateSignalSchema (types.js), (b) extraer etiquetas SSP de fuentes que las provean (Supabase climate_cells con ensemble-all-sspXXX), (c) actualizar SIGNAL_METADATA y inferScenario() en signal-metadata.js. La función y el mapping están diseñados para soportar esa extensión sin cambios estructurales.",
  ...
  async execute(input) {
    ...
    for (const entry of definitions) {
      ...
      phenomena.push({
        ...
        scenario: inferScenario(matchingSignals.map(s => s.name)),
